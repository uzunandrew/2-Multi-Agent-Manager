...
def dedup_key(f):
    """Generate dedup key from page + title keywords."""
    page = f.get('page', 0)
    title = f.get('title', '').lower()
    # Normalize: remove punctuation, take first 50 chars
    title_norm = re.sub(r'[^\w\s]', '', title)[:50].strip()
    return f'{page}_{title_norm}'
