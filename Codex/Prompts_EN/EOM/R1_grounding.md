...
For each finding, default to: `claim_basis: "mixed"`, `norm_role: "core"`, `requires_exact_quote: true`.
Downgrade `norm_role` to `supporting` when the issue is not a true violation but incomplete documentation, equipment completeness, or a recommendation.
