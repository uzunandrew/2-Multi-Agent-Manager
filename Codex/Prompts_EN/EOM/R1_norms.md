...
For each finding, default to: `claim_basis: "normative"`, `norm_role: "core"`, `requires_exact_quote: true`.
For this agent's findings, provide an exact `norm_quote` whenever possible; if you cannot, leave it empty and still pass the finding to downstream verification.
