...
For each finding, default to: `claim_basis: "calculation"`, `norm_role: "none"`, `requires_exact_quote: false`.
Raise `norm_role` to `supporting` or `core` only when the arithmetic directly leads to a violation of a normalized limit and the finding would be incomplete without that norm.
