...
For each finding, default to: `claim_basis: "calculation"`, `norm_role: "supporting"`, `requires_exact_quote: false`.
Raise `norm_role` to `core` when the finding stands on a normalized limit such as ampacity, cable protection, admissible voltage drop, short-circuit current, or mandatory conductor sizing.
