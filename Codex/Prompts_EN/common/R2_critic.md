...
- `_output/critic_payload.json` — compact findings from all agents (built by build_critic_payload.py), including a short summary from `norm_finding_checks`
- `document_enriched.md` — full text + drawing descriptions (for evidence verification)
- `_output/norms_subset.json` — for verifying normative references (subset of norms_db, built by build_norms_subset.py)
...
1. First inspect the following fields in `critic_payload.json`:
   - `norm_role`
   - `claim_basis`
   ...
   - `corrected_norm_ref`
   - `norm_quote_preview`
2. Use these fields as the **primary signal** from the `norm_finding_checks` preprocessing step
3. Re-check the document in `_output/norms_subset.json` when needed
4. Is the norm active? (`status: active`)
5. Does the referenced clause match the content of the finding?
6. Is `norm_confidence` adequate? (if the agent assigned 0.95, but the clause is questionable → flag it)

**Rule for how strongly the finding depends on the norm:**
- `norm_role = "none"` → a weak or missing norm by itself should not kill the finding
- `norm_role = "supporting"` → if the technical substance is confirmed, `pass_weak_norm` is acceptable
- `norm_role = "core"` → if the norm is not supported, do not assign a plain `pass`
- `requires_exact_quote = true` → without an exact quote, do not assign a plain `pass` unless the evidence is otherwise overwhelming and you also set `needs_human_review`
...
|-------|------|-------------|
| `corrected_norm_ref` | string \| null | When verdict is `pass_weak_norm` — provide the correct norm reference you found during verification |
| `corrected_category` | string \| null | When the finding belongs in a different category (`"Критическое"`, `"Экономическое"`, `"Эксплуатационное"`) |
| `duplicate_of` | string \| null | When verdict is `duplicate` — the `temp_id` of the better/more detailed finding |
| `needs_human_review` | bool | `true` when you cannot reach a confident verdict — borderline cases, conflicting evidence |
| `norm_review_status` | string | Final normative verification result: `verified`, `corrected`, `weak_support`, `unsupported`, `not_needed`, `needs_human_review` |
| `norm_support_level` | string | How strongly the norm supports the finding: `direct`, `partial`, `context_only`, `unsupported` |
| `norm_doc_status` | string | Aggregated document status: `active`, `replaced`, `cancelled`, `unknown`, `mixed` |
| `norm_quote_used` | string | Short exact quote used in the decision, if available |

These fields are **optional** — set to `null` / `false` when not applicable.
